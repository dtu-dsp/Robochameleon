function [u1x,u1y] = manakov(u0x,u0y,dt,dz,nz,alphaa,alphab,betapa,betapb,gamma,psp,method,maxiter,tol);
...
if strcmp(method,'circular')   %% CIRCULAR BASIS METHOD %%
  ...
  for iz = 1:nz,
    ...
    for ii = 1:maxiter,
      ...
      % Calculate nonlinear section (MANAKOV aproximation: modified by
      % Edson (26/11/2014):
      uva = uahalf .* exp( (-1i*(8/9)*gamma*dz).* ...
                           ( 2*(abs(u0a).^2+abs(u1a).^2)/2 + ...
                             2*(abs(u0b).^2+abs(u1b).^2)/2 ) );
      uvb = ubhalf .* exp( (-1i*(8/9)*gamma*dz).* ...
                           ( 2*(abs(u0b).^2+abs(u1b).^2)/2 + ...
                             2*(abs(u0a).^2+abs(u1a).^2)/2 ) );
      uva = fft(uva);
      uvb = fft(uvb); 
