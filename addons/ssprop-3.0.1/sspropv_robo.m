...
%
% Matlab script modified for speed by:
...
if strcmp(method,'circular')   %% CIRCULAR BASIS METHOD %%
  ...
  u0a = (u0x+1j*u0y)/sqrt(2);
  u0b = (u0y+1j*u0x)/sqrt(2);
  
  % Propagation matrix for linear calcuations
  
  h11 = ( (1+sin(2*chi))*ha + (1-sin(2*chi))*hb )/2;
  h12 = -1j*exp(2j*psi)*cos(2*chi)*(ha-hb)/2;
  h21 = 1j*exp(-2j*psi)*cos(2*chi)*(ha-hb)/2;
  ...
  for iz = 1:nz
    ...
    for ii = 1:maxiter
        ...
        uva = uahalf .* exp( -1j/3*gamma*dz .* (pa+2*pb) );
        uvb = ubhalf .* exp( -1j/3*gamma*dz .* (pb+2*pa) );
        uva = fft(uva);
        uvb = fft(uvb);
        ...
        if (sum(abs(dua).^2)+sum(abs(dub).^2))/sum(Pu1a+Pu1b) < tol_sq
            warning('Failed to converge to %f in %d iterations',tol,maxiter);
            break;
        end
  ...
  u1x = (u1a-1j*u1b)/sqrt(2);
  u1y = (u1b-1j*u1a)/sqrt(2);
